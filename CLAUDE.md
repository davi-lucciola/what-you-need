...
- **Products** (`app/core/agents/products/`): `build_product_search_agent` compiles a **turn-based** subgraph (`collect_requirements → plan_search → execute_search → validate_products → present_recommendations`). No `interrupt()`: `collect_requirements` uses **next-best-question** (extracts requirements from the messages, asks the single highest-value missing field, and ends the turn by setting `phase`); once complete, the same turn flows through a **planner/executor** search (`plan_search` generates queries, `execute_search` runs them concurrently and extracts candidates), a `validate_products` reflection (budget filter **+ online-reviews check**), an optional replan loop, and `present_recommendations`, which attaches purchase links to each of the `TOP_PRODUCTS` (5) products. `tools.py` runs the Tavily searches; `nodes.py` holds the subgraph steps + routers.

### API and services layer

- **API** (`app/api/`): the single endpoint is `POST /threads/{thread_id}/chat` (`routers/chat.py`), returning an `EventSourceResponse` (SSE). `schemas/chat.py` holds `ChatRequest` (`{ message }`); `deps.py` exposes `get_agent`, which reads `app.state.agent`. The router is **turn-based**: every message is fed as a fresh `HumanMessage` (with `next` reset) and the graph runs START→END — there are no interrupts to resume. Multi-turn position lives in `phase` (see State), read by the supervisor.
- **Service** (`app/core/services/chat.py`): `event_stream` runs the graph with `astream(stream_mode='messages')` and yields SSE events: `token` (incremental live text, supervisor node skipped), `message` (final authoritative text — the last non-empty `AIMessage`, also covers non-LLM replies like the links-attached presentation), and always `done` last.
- **Checkpointer** (`app/infra/checkpointer.py`): `get_postgres_checkpointer_pool` yields an `AsyncPostgresSaver` over an `AsyncConnectionPool`, set up in the FastAPI lifespan. The connection string comes from `CHECKPOINTER_DATABASE_URL`. State stays server-side (stateful); with turn-based flows only `messages` + `phase` need to survive between turns.
...
- **State** (`app/core/agents/states.py`): shared `ChatState` extends LangChain's `AgentState` (so it carries `messages`) and adds `next` (transient per-turn routing, `take_latest_nonempty` reducer) and `phase` (durable multi-turn position, plain-overwrite channel so it can be cleared with `''`). `next` and `phase` are distinct: `phase` feeds the supervisor's deterministic override (via `PHASE_OWNER` in `constants.py`) that computes `next` without an LLM call while a flow is active. `phase` is a **shared channel** with `ProductSearchState`, so the subgraph's writes propagate to the parent. The product-search state (`ProductSearchState`, `RequirementsDict`, `ProductDict`) lives in `app/core/agents/products/state.py`.
...
- **Tavily** (`app/infra/tavily.py`, `app/core/agents/products/tools.py`): the cached `AsyncTavilyClient` (`get_tavily_client`) backs `run_planned_searches` + `extract_candidates` (the planner/executor split), `search_product_reviews` (validation), and `fetch_purchase_links` (attached in the presentation). The `TavilyResult` TypedDict mirrors the Tavily API shape.
