...
from app.core.agents.products import agent as products_nodes
from app.core.agents.products.agent import PRESENT_TOOL, product_agent_node
...
def _patch_agent(mocker: MockerFixture, new_messages: list[Any]) -> Any:
    mocker.patch.object(products_nodes, 'get_llm', return_value=mocker.Mock())
    agent = _fake_agent(mocker, new_messages)
    create_agent = mocker.patch.object(
        products_nodes, 'create_agent', return_value=agent
    )
    return create_agent
...
async def test_question_turn_keeps_phase_active(mocker: MockerFixture) -> None:
    ...
    result = await product_agent_node(state)
